# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# This file is the source Rails uses to define your schema when running `bin/rails
# db:schema:load`. When creating a new database, `bin/rails db:schema:load` tends to
# be faster and is potentially less error prone than running all of your
# migrations from scratch. Old migrations may fail to apply correctly if those
# migrations use external dependencies or application code.
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema[7.0].define(version: 2022_12_05_231824) do
  create_table "produtos", charset: "utf8mb4", collation: "utf8mb4_0900_ai_ci", force: :cascade do |t|
    t.string "nome"
    t.string "descricao"
    t.integer "preco"
    t.integer "tamanho"
    t.date "prazo"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
  end

  create_table "senha_de_segurancas", charset: "utf8mb4", collation: "utf8mb4_0900_ai_ci", force: :cascade do |t|
    t.bigint "usuario_id", null: false
    t.string "senha"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["usuario_id"], name: "index_senha_de_segurancas_on_usuario_id"
  end

  create_table "usuarios", charset: "utf8mb4", collation: "utf8mb4_0900_ai_ci", force: :cascade do |t|
    t.string "nome"
    t.string "sobrenome"
    t.boolean "telefone"
    t.string "email"
    t.string "senha"
    t.boolean "status_adm"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.string "ndeverificacao"
    t.string "status_usuario"
  end

  add_foreign_key "senha_de_segurancas", "usuarios"
end
